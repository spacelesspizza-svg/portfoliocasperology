...
function CaseDescriptionBlock({ text }: { text: string }) {
  return (
    <div className="mt-[48pt] w-full">
      <Typography
        as="p"
        className="w-full whitespace-pre-line text-pretty text-base leading-tight text-neutral-600 md:text-lg"
      >
        {text}
      </Typography>
    </div>
...
const projects: Project[] = [
  {
    ...
    descriptionAfterGalleryRow1: `цвета выбраны в соответствии с основными гайдами вб, но с ноткой серьезности.
...
>150 презентаций >120 учебных материалов за последние полгода`,
    ...
    caseGallerySlideshow: {
      6: [
        "/images/pro-wb-block-6-a.png",
        "/images/pro-wb-block-6-b.png",
...
>110 презентаций >40 учебных материалов за последние полгода er - 7.5%`,
  ...
  },
  {
    ...
    descriptionAfterCover: `задача: разработать визуал, который не считывался бы как помощь конкретному маркетплейсу. специально для этого был подобран оранжевый цвет — он не ассоциируется с айдентикой ни одного из нынешних брендов e-сom гигантов.`,
  ...
  },
  {
    ...
    description: `спиритизм — нишевый магазин одежды, выходящий на рынок берлина. хозяева большие фанаты эзотерики и хотели отразить это в айдентике своего бренда`,
    descriptionAfterCover: `задачей было разработать узнаваемый стиль для грандж-магазина, но не уйти в слишком сильную "панк" эстетику.`,
    ...
    coverSize: "2048×1280",
    mediaType: "image",
    image: "/images/spiritism-cover-2048.png",
    caseGallery: {
...
function CaseSection({ project, index }: { project: Project; index: number }) {
  return (
    <section id={index === 0 ? "work" : project.title} className="page-px py-16 md:py-24">
      <div className="w-full">
        ...
        <div className="w-full">
          <h2 className="w-full text-4xl font-black uppercase leading-[0.88] tracking-tighter sm:text-5xl md:text-6xl md:leading-[0.85] lg:text-8xl">
            {project.title}
          </h2>
          <Typography
            as="p"
            className="mt-6 w-full whitespace-pre-line text-pretty text-base leading-tight text-neutral-600 md:mt-8 md:text-lg"
          >
            {project.description}
          </Typography>
          <div className="mt-10 grid w-full grid-cols-[90px_1fr] gap-y-2 text-xs uppercase md:grid-cols-[120px_1fr]">
            ...
            {project.coverSize ? (
              <>
                <span className="text-neutral-400">заставка 1</span>
                <span>{project.coverSize}</span>
              </>
            ) : null}
          </div>
        </div>
      ...
      <div className="mt-[48pt] aspect-[2048/1280] w-full overflow-hidden bg-neutral-200">
        {project.mediaType === "video" && project.video ? (
      ...
      <div
        className={`grid w-full grid-cols-2 gap-2 md:gap-3 ${project.descriptionAfterCover ? "mt-[48pt]" : "mt-2 md:mt-3"}`}
      >
      ...
      <div
        className={`grid w-full grid-cols-2 gap-2 md:gap-3 ${project.descriptionAfterGalleryRow1 ? "mt-[48pt]" : "mt-2 md:mt-3"}`}
      >
      ...
      <div
        className={project.descriptionAfterGalleryRow2 ? "mt-[48pt]" : "mt-2 md:mt-3"}
      >
        {project.caseGallerySlideshow?.[6] ? (
          <CaseGallerySlideshow frames={project.caseGallerySlideshow[6]} intervalMs={5000} aspect="2048/1280" />
        ) : (
...
export default function PortfolioSite() {
  return (
    <main className="min-h-screen bg-[#f4f3ef] text-neutral-950 selection:bg-neutral-950 selection:text-white">
      <nav className="page-px fixed left-0 right-0 top-0 z-50 flex items-center justify-between bg-[#f4f3ef] py-4 text-[10px] font-medium uppercase leading-tight tracking-tight text-neutral-950 md:py-5 md:text-xs">
        <a href="http://t.me/casperology" className="text-[10px] md:text-xs">
          @casperology
        </a>
      ...
      <section id="top" className="page-px relative min-h-screen overflow-hidden pt-24 md:pt-28">
        <div className="grid min-h-[calc(100vh-7rem)] grid-cols-1 items-stretch gap-6 lg:grid-cols-[1fr_1.15fr]">
          <div className="flex flex-col">
            <div>
              ...
              <Typography
                as="p"
                className="mt-6 text-base leading-tight text-neutral-400 md:mt-8 md:text-lg"
              >
                {profile.intro}
              </Typography>
            </div>
      ...
      <footer className="page-px flex flex-col gap-6 border-t border-neutral-300 py-8 text-sm md:flex-row md:items-end md:justify-between">
        <span>© 2026</span>
        <a
